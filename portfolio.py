...
class portfolio(object):
  ...
  def generate_naive_order(self, signal):
    ...
    if direction == 'long' and current_qty == 0:
      print(f"Date: {self.handler.current_date} -- ticker {ticker}: buy, {price:.2f}, {market_qty}")
      order = order_event(ticker, market_qty, 'buy', price)
    if direction == 'short' and current_qty == 0:
      print(f"Date: {self.handler.current_date} -- ticker {ticker}: sell, {price:.2f}, {market_qty}")
      order = order_event(ticker, market_qty, 'sell', price)
    if direction == 'exit' and current_qty > 0:
      print(f"Date: {self.handler.current_date} -- ticker {ticker}: sell, {price:.2f}, {abs(current_qty)}")
      order = order_event(ticker, abs(current_qty), 'sell', price)
    if direction == 'exit' and current_qty < 0:
      print(f"Date: {self.handler.current_date} -- ticker {ticker}:buy, {price:.2f}, {abs(current_qty)}")
